  ...
  if (val) {
    nextTick(() => {
      if (editorRef.value) {
        ...
        // Protect only truly opaque elements (like refs, math) from direct editing
        const opaque = editorRef.value.querySelectorAll('[typeof^="mw:Extension"]')
        opaque.forEach(el => {
          el.setAttribute('contenteditable', 'false')
        })

        // Ensure links and transclusions are explicitly editable to prevent skipping
        const editableParts = editorRef.value.querySelectorAll('[rel="mw:WikiLink"], [typeof="mw:Transclusion"]')
        editableParts.forEach(el => {
...
function handleBlur(e: Event) {
  ...
  // Sync edited text back to Wikipedia template data (data-mw)
  const transclusions = el.querySelectorAll('[typeof="mw:Transclusion"]')
  transclusions.forEach(trans => {
    try {
      const dataMwAttr = trans.getAttribute('data-mw')
      if (!dataMwAttr) return
      const data = JSON.parse(dataMwAttr)
      const part = data.parts?.[0]?.template
      
      // Specifically target {{ill}} templates generated by our tool or Parsoid
      if (part?.target?.wt?.toLowerCase() === 'ill' && part.params) {
        const newText = (trans as HTMLElement).innerText.trim()
        
        if (part.params['label']) {
          // If a label exists, only update the label
          part.params['label'].wt = newText
        } else if (part.params['1']) {
          // Otherwise, update the target Japanese title
          part.params['1'].wt = newText
        }
        
        trans.setAttribute('data-mw', JSON.stringify(data))
      }
    } catch (err) {
      // Silently catch errors during sync to prevent app crash
    }
  })
